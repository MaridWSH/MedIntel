...
function Question({
  ...
  return (
    <fieldset className="rounded-xl sm:rounded-2xl border border-ink/10 bg-paper p-4 sm:p-5 md:p-7">
      <legend className="sr-only">{title}</legend>
      <div className="flex items-start gap-3 sm:gap-4 mb-4 sm:mb-5">
        <span className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl bg-ink text-paper inline-flex items-center justify-center text-[10px] mono-stat shrink-0">
          {number}
        </span>
        <div>
      ...
      <main className="min-h-screen bg-paper-warm/35">
        <section className="relative overflow-hidden border-b border-ink/10">
          ...
          <div className="max-w-[980px] mx-auto px-4 sm:px-6 py-10 sm:py-14 md:py-20 relative">
            <div className="text-[10px] sm:text-[10.5px] mono-stat text-teal-deep mb-3 sm:mb-4">BETA RESEARCH · 3–5 MINUTES</div>
            ...
            <p className="serif-body text-[15px] sm:text-[16px] md:text-[18px] text-ink-soft leading-relaxed mt-4 sm:mt-5 max-w-[720px]">
              ...
              Responses are anonymous and will guide the CiteRounds beta.
            </p>
        ...
        <section className="max-w-[980px] mx-auto px-4 sm:px-6 py-7 sm:py-10 md:py-14">
          {submitted ? (
            <div className="rounded-2xl sm:rounded-3xl border border-teal-deep/20 bg-paper p-6 sm:p-8 md:p-12 text-center shadow-sm">
              ...
              <h2 className="display text-[30px] sm:text-[34px] mt-6">Thank you for sharing your workflow.</h2>
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-3 mt-7">
                <Link href="/search" className="btn-primary px-5 py-3 rounded-xl bg-ink text-paper">Explore CiteRounds</Link>
                <Link href="/feedback" className="px-5 py-3 rounded-xl border border-ink/15">Rate the website</Link>
              </div>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5" noValidate>
              <div className="rounded-xl sm:rounded-2xl border border-ink/10 bg-paper p-4 sm:p-5 md:p-7">
              ...
              <Question number="01" title="What sources do you usually rely on when searching for medical research or references?" hint="Select all that apply.">
                <div className="grid sm:grid-cols-2 gap-2.5">
                  {SOURCES.map((source) => (
                    <label key={source} className={`flex min-h-12 items-center gap-3 rounded-xl border px-3 sm:px-4 py-3 cursor-pointer transition ${sources.includes(source) ? 'border-teal-deep bg-teal-deep/[0.06]' : 'border-ink/10 hover:border-ink/25'}`}>
              ...
              <Question number="02" title="When looking for a specific answer, how many papers do you usually need to read before reaching it?">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-2.5">
                  {PAPERS_NEEDED.map((item) => (
                    <label key={item} className={`flex min-h-12 items-center justify-center rounded-xl border px-2 sm:px-4 py-3 text-center cursor-pointer transition ${papersNeeded === item ? 'border-teal-deep bg-teal-deep/[0.06]' : 'border-ink/10 hover:border-ink/25'}`}>
              ...
              <Question number="03" title="Which part of reviewing research takes most of your time?">
                <div className="grid sm:grid-cols-2 gap-2.5">
                  {TIME_TASKS.map((item) => (
                    <label key={item} className={`flex min-h-12 items-center gap-3 rounded-xl border px-3 sm:px-4 py-3 cursor-pointer transition ${timeTask === item ? 'border-teal-deep bg-teal-deep/[0.06]' : 'border-ink/10 hover:border-ink/25'}`}>
              ...
              <Question number="04" title="What is the biggest problem you face when searching for medical research or references?">
                <div className="grid sm:grid-cols-2 gap-2.5">
                  {PROBLEMS.map((item) => (
                    <label key={item} className={`flex min-h-12 items-center gap-3 rounded-xl border px-3 sm:px-4 py-3 cursor-pointer transition ${problem === item ? 'border-teal-deep bg-teal-deep/[0.06]' : 'border-ink/10 hover:border-ink/25'}`}>
              ...
              <Question number="05" title="If a platform provided an accurate summary with the key findings and essential points, would you rely on it? Why or why not?">
                <div className="grid grid-cols-3 gap-2.5 mb-4">
                  {['Yes', 'Maybe', 'No'].map((item) => (
                    <label key={item} className={`flex min-h-12 items-center justify-center rounded-xl border px-2 sm:px-4 py-3 text-center cursor-pointer transition ${trust === item ? 'border-teal-deep bg-teal-deep/[0.06]' : 'border-ink/10 hover:border-ink/25'}`}>
